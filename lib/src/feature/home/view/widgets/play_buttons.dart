...
class PlayButtonsSearch extends StatelessWidget {
  final String path;
  final VoidCallback? onTap;
  const PlayButtonsSearch({super.key, required this.path, this.onTap});
    ...
    return Padding(
      ...
      child: Row(
        children: [
          MaterialButton(
            onPressed: () {
              context.go("${AppRouteName.mainPage}${AppRouteName.searchPage.substring(1)}/${AppRouteName.bookSearchPage}/${AppRouteName.pdfPage}");
            },
            height: 55.h,
            minWidth: 150.w,
            shape: RoundedRectangleBorder(
...
class LibraryPdfViewerPage extends StatelessWidget {
  const LibraryPdfViewerPage({super.key});
